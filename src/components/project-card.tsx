...
export function ProjectCard({ title, description, image, tags, href, githubHref }: ProjectCardProps) {
    ...
    >
      ...
      >
        <div className="flex h-full flex-col justify-end p-6">
          ...
          <p 
            className="mt-2 text-gray-300 transform-gpu"
            style={{ transform: 'translateZ(3px)' }}
          >
            {description}
          </p>
          <div 
            className="mt-4 flex flex-wrap gap-2 transform-gpu"
            style={{ transform: 'translateZ(3px)' }}
          >
            {tags.map((tag) => (
              <span
