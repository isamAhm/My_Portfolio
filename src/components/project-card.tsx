...
export const ProjectCard = memo(function ProjectCard({
  title, description, image, tags, href, githubHref, onPlayClick,
}: ProjectCardProps) {
        ...
        <div className="flex h-full flex-col justify-end p-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {githubHref && (
                <a href={githubHref} target="_blank" rel="noopener noreferrer" aria-label="View source code"
              ...
              {onPlayClick && (
                <button onClick={onPlayClick} aria-label="Play project demo"
                  className="flex items-center gap-2 text-sm text-gray-300 hover:text-blue-400 transition-colors duration-200 bg-black/80 rounded p-2 backdrop-blur-lg"
                  style={{ transform: 'translateZ(3px)' }}>
          ...
          <div className="mt-4 flex flex-wrap gap-2 transform-gpu" style={{ transform: 'translateZ(3px)' }}>
            {tags.map((tag) => (
              <span key={tag} className="rounded-full bg-black/85 backdrop-blur-lg px-3 py-1 text-sm text-gray-300">
                {tag}
              </span>
            ))}
          </div>
